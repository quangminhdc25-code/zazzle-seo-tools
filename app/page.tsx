        ...
        <div className="flex flex-col xl:flex-row gap-5 items-start">
          ...
          <form onSubmit={submit} className="flex-1 w-full xl:w-[40%] flex flex-col gap-5">
                ...
                <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                  {amzItems.map((item, i) => (
                    <div key={i} className="bg-white/70 dark:bg-[#1c1c1e]/50 rounded-xl p-4 border border-slate-200/50 dark:border-white/5 shadow-sm">
                      ...
                      <textarea className="w-full bg-transparent text-sm font-medium outline-none min-h-[60px] mt-2 resize-y placeholder-slate-400 custom-scrollbar" value={item.description} onChange={e => { const n = [...amzItems]; n[i].description = e.target.value; setAmzItems(n); }} placeholder="Description" />
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                  {etsyItems.map((item, i) => (
                    <div key={i} className="bg-white/70 dark:bg-[#1c1c1e]/50 rounded-xl p-4 border border-slate-200/50 dark:border-white/5 shadow-sm">
                      ...
                      <input className="w-full bg-transparent border-b border-slate-200 dark:border-slate-700 pb-1.5 text-sm font-semibold outline-none focus:border-pink-500 transition-colors placeholder-slate-400" value={item.title} onChange={e => { const n = [...etsyItems]; n[i].title = e.target.value; setEtsyItems(n); }} placeholder="Title" />
            ...
            <button type="submit" disabled={loading} className={`w-full py-3.5 rounded-xl font-bold text-sm text-white shadow-md transition-all ${loading ? 'bg-slate-400 cursor-wait' : 'bg-blue-600 hover:bg-blue-700 active:scale-[0.99]'}`}>
              {loading ? 'Processing via Advanced LPU...' : 'Generate Listing'}
            </button>
                    ...
                    return (
                      <div key={i} className="p-5 md:p-6 rounded-2xl bg-white/80 dark:bg-[#3a3a3c]/60 backdrop-blur-md border border-slate-200/50 dark:border-white/5 shadow-sm relative group transition-all">
                        <div className="flex justify-between items-center mb-4 border-b border-slate-100 dark:border-slate-700 pb-3">
                          <div className="flex items-center gap-3"><span className="text-sm font-bold text-emerald-600 dark:text-emerald-400">#{results.length - i}</span>{i === 0 && <span className="bg-emerald-600 text-white text-[9px] font-bold px-2.5 py-1 rounded-md uppercase tracking-wider">Mới nhất</span>}</div>
                        </div>
                        <div className="space-y-4">
                          <div className="bg-slate-50/80 dark:bg-[#2c2c2e]/60 p-4 rounded-xl border border-slate-100 dark:border-white/5">
                            <div className="flex justify-between items-center mb-2"><span className="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">SEO Title</span><CopyBtn text={v.newTitle} /></div>
                            <p className="text-base font-bold leading-tight">{v.newTitle}</p>
                          </div>
                          <div className="bg-slate-50/80 dark:bg-[#2c2c2e]/60 p-4 rounded-xl border border-slate-100 dark:border-white/5">
                            <div className="flex justify-between items-center mb-2"><span className="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Storytelling Description</span><CopyBtn text={v.newDescription} /></div>
                            <p className="text-sm font-medium text-slate-600 dark:text-slate-300 leading-relaxed">{v.newDescription}</p>
                          </div>
                          <div className="bg-slate-50/80 dark:bg-[#2c2c2e]/60 p-4 rounded-xl border border-slate-100 dark:border-white/5">
                            <div className="flex justify-between items-center mb-2"><span className="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Unique Tags (10)</span><CopyBtn text={v.newTags} /></div>
