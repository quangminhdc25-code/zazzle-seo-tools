...
export default function ZazzleSEOTool() {
  ...
  // Tự động Load lịch sử từ LocalStorage khi khởi động
  useEffect(() => {
    const saved = localStorage.getItem('zazzle_history');
    if (saved) {
      try { setResults(JSON.parse(saved)); } catch (e) {}
    }
  }, []);

  // Thuật toán nén ảnh Canvas
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!coreSubject) return setError('Vui lòng nhập Core Subject.');
    ...
    try {
      ...
      if (data.variants) {
        const newHistory = [...data.variants, ...results];
        setResults(newHistory);
        // Lưu lịch sử (Chỉ lưu text, không lưu Base64 để tránh đầy bộ nhớ)
        localStorage.setItem('zazzle_history', JSON.stringify(newHistory.slice(0, 20))); 
  ...
  const clearHistory = () => {
    setResults([]);
    localStorage.removeItem('zazzle_history');
  };

  return (
    <div className="max-w-4xl mx-auto p-6 font-sans">
      <h1 className="text-3xl font-bold mb-8 text-center text-blue-800 flex justify-center items-center gap-3">
        Zazzle Hybrid SEO Pro <span className="text-sm bg-blue-100 text-blue-800 py-1 px-3 rounded-full border border-blue-300">v2.0 Enterprise</span>
      </h1>
      ...
      <form onSubmit={handleGenerate} className="bg-white shadow-lg rounded-xl px-8 pt-6 pb-8 mb-8 border border-gray-200">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          ...
          <div 
            ...
            className={`p-4 rounded-lg border-2 border-dashed transition-colors ${isDragging ? 'bg-blue-100 border-blue-500' : 'bg-blue-50 border-blue-200'}`}
          >
            <label className="block text-blue-900 font-bold mb-2">1. Upload Ảnh (Kéo Thả vào đây)</label>
            <div className="flex flex-col items-center justify-center py-2">
              <label className="cursor-pointer bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded shadow-sm text-sm">
                Chọn Ảnh Hoặc Kéo Thả
                <input type="file" accept="image/*" onChange={handleImageUpload} className="hidden" />
              </label>
            </div>
            {imageBase64 && <div className="mt-3 text-center"><img src={imageBase64} alt="Preview" className="h-20 object-contain mx-auto rounded border shadow-sm bg-white" /></div>}
          </div>

          <div className="p-4 bg-blue-50 rounded-lg border border-blue-100">
            <label className="block text-blue-900 font-bold mb-2">2. Text in trên áo (OCR Bypass)</label>
            <input type="text" placeholder="Gõ chính xác chữ trên áo..." className="w-full p-2 border border-gray-300 rounded bg-white text-black font-medium focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none" value={textOnDesign} onChange={(e) => setTextOnDesign(e.target.value)} />
          </div>
        </div>

        {/* Khối Scraping Etsy */}
        <div className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
          <label className="block text-gray-800 font-bold mb-2">3. Auto Scrape Etsy (Dán Link Etsy hoặc nhập Text)</label>
          <input type="text" placeholder="VD: https://www.etsy.com/listing/..." className="w-full p-2 border border-gray-300 rounded bg-white text-black font-medium focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none" value={etsyInput} onChange={(e) => setEtsyInput(e.target.value)} />
        </div>
        ...
        <div className="mb-6 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input type="text" placeholder="VD: Messy Bun..." className="w-full p-2 border border-gray-300 rounded mt-1 bg-white text-black font-medium focus:border-yellow-500 outline-none" value={coreSubject} onChange={(e) => setCoreSubject(e.target.value)} required />
            </div>
            <div>
              <label className="text-sm font-bold text-gray-800">Audience</label>
              <input type="text" placeholder="VD: Mom, Union..." className="w-full p-2 border border-gray-300 rounded mt-1 bg-white text-black font-medium focus:border-yellow-500 outline-none" value={targetAudience} onChange={(e) => setTargetAudience(e.target.value)} />
            </div>
            <div>
              <label className="text-sm font-bold text-gray-800">Vibe / Occasion</label>
              <input type="text" placeholder="VD: Labor Day..." className="w-full p-2 border border-gray-300 rounded mt-1 bg-white text-black font-medium focus:border-yellow-500 outline-none" value={vibe} onChange={(e) => setVibe(e.target.value)} />
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mt-8">
          ...
          <button type="submit" disabled={loading} className={`bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded shadow-md ${loading ? 'opacity-50' : ''}`}>
            {loading ? 'Hệ thống đang xử lý...' : 'Tạo Nội Dung SEO'}
          </button>
        </div>
      </form>
      ...
      {/* HIỂN THỊ KẾT QUẢ VÀ NÚT CLEAR */}
      {results.length > 0 && (
        <div className="mb-4 flex justify-between items-center">
          <h2 className="text-xl font-bold text-gray-800">Lịch sử kết quả (Đã lưu)</h2>
          <button onClick={clearHistory} className="text-sm text-red-600 hover:text-red-800 font-bold underline">Xóa lịch sử</button>
        </div>
      )}
